    ...
    if ($action === 'edit') {
        ...
        if ($table === 'bank_accounts') {
            ...
            if ($contractEnd !== null && $contractEnd !== '' && $depositAmount > 0) {
                $stUnpaid = db()->prepare("SELECT 1 FROM payment_requests WHERE contracts_id = ? AND type NOT IN ('deposit', 'deposit_return') AND valid_to IS NULL AND paid_at IS NULL LIMIT 1");
